...
import oss2
import uuid
from kgce import Action, ActionOutput, BackendModel, BackendOutput, Message, MessageType
from kgce.agents.utils import extract_text_and_code_prompts
...
class QwenVLModel(BackendModel):
    def __init__(
            self,
            model: str,
    ...
    def _call_api(
        ...
        # print("call_api的response:",response)
...
class QwenVLModelJSON(QwenVLModel):
    def __init__(
            self,
            model: str,
...
class SGlangQwenVLModelJSON(QwenVLModelJSON):
    def _construct_new_message(self, message: list[Message]) -> dict[str, Any]:
        new_message_content: list[dict[str, Any]] = []
        image_count = 0
        for _, msg_type in message:
